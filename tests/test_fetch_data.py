...
def test_process_local_data(
    mock_geocodificar, mock_glob, mock_read_csv, mock_ibis, tmp_path
):
...
def test_fetch_all_states_writes_csvs(tmp_path, requests_mock):
    ...
    for uf in ("RO", "SP"):
        requests_mock.get(url_base.format(uf), content=CAIXA_CSV.encode("latin-1"))
...
def test_fetch_all_states_fails_on_empty_state(tmp_path, requests_mock):
    ...
    requests_mock.get(url_base.format("SP"), content="\n".join(empty).encode("latin-1"))
...
def test_fetch_state_sends_coherent_browser_headers(requests_mock):
    ...
    fetch_state("RO", url_base="https://exemplo.test/Lista_imoveis_{}.csv", jitter=None)
