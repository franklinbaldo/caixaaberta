...
artifact: output_data/imoveis_geocoded_AAAA-MM-DD.parquet
...
| `latitude`      | float | Nulo quando nem o município casa com o CNEFE. Ver `precisao` antes de usar. |
| `longitude`     | float | Idem. |
| `precisao`      | texto | Nível em que a [geocodificação](geocodificacao.md) casou: `logradouro_localidade`, `logradouro`, `localidade` ou `municipio`. Sem ela, coordenada de rua e centro de município são indistinguíveis — ver [geocodificado não é localizado](armadilhas/geocodificado-nao-e-localizado.md). |
| `desvio_metros` | float | Incerteza da coordenada, do próprio CNEFE. |
| `scrape_date`   | data  | Data em que o Caixa Aberta observou a fonte. É a identidade temporal canônica do snapshot e é igual para todas as linhas do arquivo. |

`preco`, `avaliacao` e `desconto` chegam da Caixa em formato brasileiro
(`99.743,11`) e são convertidos para float no download.

A `Data de geração` declarada pela Caixa no preâmbulo dos CSVs é preservada no
ZIP bruto como proveniência secundária. Ela não substitui `scrape_date`, que é
a data controlada pelo próprio scraping do Caixa Aberta.
